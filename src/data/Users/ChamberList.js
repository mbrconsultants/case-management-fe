...
const handleShowEditModal = (chamber) => {
  ...
  setValue("email2", chamber.email2);
  setValue("phone", chamber.phone);
  setValue("phone2", chamber.phone2);
  ...
  const lawyers = chamber.ChamberLawyers[0]?.lawyer_name
    ...
    : [];
  reset({ lawyers: lawyers.map((lawyer) => ({ name: lawyer.name, phone: lawyer.phone })) });
...
const updateChamber = async (formData) => {
  ...
  try {
    ...
    data.append(
      "lawyer_name",
      JSON.stringify(formData.lawyers.map((lawyer) => ({ name: lawyer.name, phone: lawyer.phone })))
    );
  ...
  <Modal.Body>
    ...
    >
      ...
      <CCol md={6}>
        ...
        <CFormInput
          defaultValue={currentChamber ? currentChamber.email2 : ""}
          id="email2"
      ...
      <CCol md={6}>
        ...
        <CFormInput
          defaultValue={currentChamber ? currentChamber.phone2 : ""}
          id="phone2"
      ...
      <CCol md={12}>
        {fields.map((lawyer, index) => (
          <Row key={lawyer.id} className="d-flex mb-2">
            <Col md={6}>
              <CFormLabel htmlFor="lawyers">Lawyer Name</CFormLabel>
              <CFormInput
            ...
            <Col md={6}>
              <CFormLabel htmlFor="lawyerPhone">Lawyer Phone</CFormLabel>
              <CFormInput
                {...register(`lawyers.${index}.phone`, { required: true })}
                type="text"
                className="me-2"
              />
            </Col>
