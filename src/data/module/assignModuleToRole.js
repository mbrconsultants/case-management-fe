...
export const AssignModuleToRole = () => {
  ...
  const getAlreadyAssigned = async (role_id) => {
    ...
    if (role_id) {
      await endpoint
        .get(`/assignsubmodule/list/${role_id}`)
        .then((res) => {
          console.log("already assigned", res.data.data);
          setAlreadyAssigned(
            res.data.data.map(({ submodule_id }) => submodule_id)
          );
          if (res.data.data == "") {
  ...
  const handleSubmit = async (e) => {
    ...
    await endpoint
      .post(`/assignsubmodule/create/multiple`, { submodule_id, role_id })
      .then((res) => {
        setIsLoading(false);
                                        ...
                                        >
                                          <td>
                                            ...
                                            <label className="switch">
                                              {alreadyAssigned &&
                                              alreadyAssigned.includes(s.id) ? (
                                                <input
                                                  type="checkbox"
                                                  checked
                                                  value={s.id}
                                                  onChange={(e) => {
                                                    toggleAlreadyChecked(
                                                      s.id,
                                                      e.target.checked
                                                    );
